(function (global, factory) {
    ...
    * @param {string} path
    */
   function addSupportMethods($class, methods, path) {
   ...
   function transformCodeToUMD(mods) {
       if (!config.config.useExternalHelpers && isBrowser) {
           babelHelpersModules.forEach(function(runtime) {
               mods.push({module: runtime, source: todo[runtime]});
           });
       }
       
       var params = mods.map(({source, module}) => {
           if (!config.config.useExternalHelpers && isNode) source = replaceBabelModule(source);
